...
fn parse_stdlib_patterns() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse stdlib/patterns.intent: {:?}",
        result.err()
    );
}
...
fn parse_stdlib_types() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse stdlib/std.intent: {:?}",
        result.err()
    );
}
...
fn count_stdlib_patterns() {
    ...
    let pattern_count = top_levels
        .iter()
        .filter(|t| matches!(t, parser::ast::TopLevel::Pattern(_)))
        .count();
    assert_eq!(
        pattern_count, 19,
        "Expected 19 patterns in stdlib, found {}",
        pattern_count
    );
}
