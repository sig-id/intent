...
use crate::parser::ast::{SystemDecl, ConstraintRule, ScopeExpr, PredicateCall};
...
pub fn analyze(systems: &[SystemDecl]) -> Vec<CoverageReport> {
    systems.iter().map(|system| {
        ...
        let mut constraint_counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
        ...
        let with_structural = components.iter().filter(|c| c.has_structural_constraints).count();
        let with_behavioral = components.iter().filter(|c| c.has_behavioral_specs).count();
        let fully_specified = components.iter().filter(|c| c.has_structural_constraints && c.has_behavioral_specs).count();
        let unconstrained = components.iter().filter(|c| !c.has_structural_constraints && !c.has_behavioral_specs).count();
        let coverage_pct = if total > 0 { ((total - unconstrained) as f64 / total as f64) * 100.0 } else { 100.0 };
...
fn collect_component_refs(rule: &ConstraintRule, refs: &mut HashSet<String>) {
    match rule {
        ...
        ConstraintRule::And(a, b) | ConstraintRule::Or(a, b)
        | ConstraintRule::Implies(a, b) | ConstraintRule::Iff(a, b) => {
        ...
        ConstraintRule::Predicate(pred) => {
            match pred {
                ...
                PredicateCall::Contains { container, entities } => {
                    ...
                    for e in entities { collect_scope_refs(e, refs); }
                }
            }
        }
        ConstraintRule::Call { subject, args, .. } => {
            collect_scope_refs(subject, refs);
            for a in args { collect_scope_refs(a, refs); }
        }
...
fn collect_scope_refs(expr: &ScopeExpr, refs: &mut HashSet<String>) {
    match expr {
        ...
        ScopeExpr::EntityList(names) => {
            for n in names { refs.insert(n.clone()); }
        }
